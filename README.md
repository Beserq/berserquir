...
- **15 behavioral evals with anti-checks** — including the ones that catch over-ceremony, not just under-performance. `/run-evals` smoke-tests your installed harness. In this repo's CI, a deterministic smoke runs on 3 OSes — and an optional **LLM-judge job** replays six behavioral scenarios (guardrail refusal, fast-path anti-ceremony, domain escalation, ALIGN gating, architectural blocking, config protection) against the compiled agents whenever canonical content changes.
