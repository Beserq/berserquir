...
5. Skills (`.claude/skills/`), then area instructions
...
Claude Code has no glob-scoped instruction files — apply these manually: **before editing files matching a pattern below, load the instruction file first.**
...
Guardrail scripts in `.berserqir/hooks/` are wired natively via `.claude/settings.json`: **git-safety** (PreToolUse on Bash — no push/force/no-verify/reset --hard without explicit human authorization) · **config-protection** + **memory-validate** (PostToolUse on edits — fix the code, not the ruler) · session hooks handle memory injection, archive-before-compaction and end-of-session memory verification. Never bypass them; overrides are human-set env vars only.
